...
class LatentMASMethod:
    ...
    def _truncate_past(self, past_kv, tokens_to_keep: int):
        ...
        # transformers >=5.x DynamicCache — slice each layer's keys/values in-place
        if hasattr(past_kv, 'layers') and hasattr(past_kv, 'get_seq_length'):
            seq_len = past_kv.get_seq_length()
            if tokens_to_keep >= seq_len:
                return past_kv
            start = seq_len - tokens_to_keep
            for layer in past_kv.layers:
                layer.keys = layer.keys[..., start:, :].contiguous()
                layer.values = layer.values[..., start:, :].contiguous()
            return past_kv

        # legacy Cache with to_legacy_cache (transformers 4.x)
        if Cache is not None and isinstance(past_kv, Cache) and hasattr(past_kv, 'to_legacy_cache'):
