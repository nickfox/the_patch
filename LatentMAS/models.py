...
def _default_dtype() -> torch.dtype:
    ...
    if torch.backends.mps.is_available():
        return torch.float16  # MPS bfloat16 support is incomplete
...
def _past_length(past_key_values) -> int:
    if not past_key_values:
        return 0
    # transformers >=5.x returns DynamicCache objects
        ...
        *,
        ...
        if past_key_values is not None:
            ...
            if past_len > 0:
                ...
                generate_kwargs["attention_mask"] = torch.cat([past_mask, attention_mask], dim=-1)
            generate_kwargs["past_key_values"] = past_key_values
            # Only pass cache_position for legacy tuple caches;
            # DynamicCache manages position tracking internally
            is_dynamic = hasattr(past_key_values, 'get_seq_length') and hasattr(past_key_values, 'layers')
            if not is_dynamic:
