...
def apply_cross_realignment(hidden: mx.array, W_cross: mx.array,
    ...
    W_cross was computed on L2-normalized embeddings, so we must L2-normalize
    the input before projection to match that space. Output is then scaled
    to the receiver's target norm.

    Args:
        hidden: [1, seq, D_sender] hidden states from sender
        W_cross: [D_sender, D_receiver] cross-alignment matrix (trained on unit vectors)
        mean_sender: unused (kept for API compatibility)
        mean_receiver: unused (kept for API compatibility)
        target_norm: scalar, average embedding norm in receiver's space
    """
    h = hidden.astype(mx.float32)
    # L2-normalize to match the space W_cross was computed on
    h_norm = h / (mx.linalg.norm(h, axis=-1, keepdims=True) + 1e-8)
    # Project in normalized space
    projected = h_norm @ W_cross
    # Scale to receiver's operating norm
    proj_norm = mx.linalg.norm(projected, axis=-1, keepdims=True)
    proj_norm = mx.maximum(proj_norm, mx.array(1e-6))
    projected = projected * (target_norm / proj_norm)
